<?php 
    ...
    $sql = "SELECT *, user.user_ID, CONCAT(firstName,' ', lastName) AS fullName FROM reservation JOIN user ON reservation.user_ID = user.user_ID 
                JOIN userinfo ON userinfo.userinfo_ID = user.userInfo_ID WHERE user.user_ID = '$userid'";
    ...
    ?>
        ...
        <form method = "POST" action = "confirmres.php"> 
            ...
            <a href=""><script> alert("test")</scipt><button type="submit" class="btn btn-secondary" name = "no" data-dismiss="modal">Cancel</button></a>
        </form>
    <?php } ?>
